...
SWEP.Instructions  = "Use !changered or !changeblue to Change the Box Color"

SWEP.Spawnable      = true
SWEP.AdminOnly      = true
SWEP.UseHands      = true
SWEP.Category       = "DerMetelGamerYT"
...
function SWEP:Reload()
  ...
  self.window:SetTitle("Zones Menu")
  ...
  for i=1,#ents.GetAll() do
    if ents.GetAll()[i]:GetClass() == "luctus_zone" then
  ...
  for k,v in pairs(safezones) do
    ...
    zonebutton:SetText("  "..v:GetID().." - Distance to the zone: "..v:GetPos():Distance(LocalPlayer():GetPos()))
  ...
  local setredbtn = vgui.Create("DButton", self.window)
...
function SWEP:SecondaryAttack()
  if not self.pos_one then
    self.Owner:PrintMessage(HUD_PRINTTALK, "Please set the first point! (LMB)")
    return
  end
  self.pos_two = self.Owner:GetEyeTrace().HitPos
  if CLIENT then return end
  luctusSavezone(self.pos_one, self.pos_two, self.colorisread)
  self.pos_one = nil
  self.pos_two = nil
end
...
function SWEP:DrawHUD()
  draw.SimpleTextOutlined("LMB = To place the first point", "DermaLarge", 10, ScrH()/2.5, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 2, Color(0,0,0,255))
  draw.SimpleTextOutlined("RMB = To place and save the second point", "DermaLarge", 10, ScrH()/2.5+40, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 2, Color(0,0,0,255))
  draw.SimpleTextOutlined("R = To open the delete menu", "DermaLarge", 10, ScrH()/2.5+80, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 2, Color(0,0,0,255))
end
